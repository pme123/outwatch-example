...
object FilterTagCreator {
  ...
  lazy val createFilterTagConfs =
    Seq(
      ...
      , FilterTagConf(allSites.last, "EN OR DE", Seq("EN", "DE").map(createFilterTags.filterTag))
      , FilterTagConf(allSites.last, "EN AND DE", Seq("EN", "DE").map(createFilterTags.filterTag))
      , FilterTagConf(allSites.last, "(IT OR EN) AND DE", Seq("EN", "DE", "IT").map(createFilterTags.filterTag))
